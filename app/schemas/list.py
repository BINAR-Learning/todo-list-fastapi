...
class ListUpdate(BaseModel):
    name: str = Field(
        ..., description="Nama baru untuk daftar tugas", example="Belanja Bulanan"
    )
...
class ListResponse(BaseModel):
    ...
    userId: str = Field(
        ..., description="ID pengguna yang memiliki daftar ini", example="user123"
    )
...
class ListWithTasks(ListResponse):
    ...
    tasks: ListType["TaskResponse"] = Field(
        default=[], description="Daftar tugas dalam list ini"
    )
