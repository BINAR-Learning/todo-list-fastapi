...
class AuthService:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        ...
        if user_data.username:
            db_user_username = self.db.query(User).filter(User.username == user_data.username).first()
            if db_user_username:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
    ...
    def authenticate_user_username(self, username: str, password: str) -> Optional[User]:
        """
